...
一方、HNR(調和性)は**意外にもリーガの方が周期的・トーン的で、プレミアの方がノイズ的**という、
方向としては最初の印象とは逆の結果になった(詳細は4.6節)。
...
ゴール後2秒の解析窓に対し、約1.85秒分のデータしか確保できていない。

## 4. 分析

### 4.1 ゴールの瞬間の指定

各ゴールについて、ゴールの瞬間を以下の方法で指定した。
...
以降の各節の分析は、この基準点をもとに行う。

### 4.2 平均スペクトル分析

歓声の音色が高域寄り(明るい)か低域寄り(こもった)かを調べるため、スペクトル分析を行う。
スペクトル分析とは、音に含まれる周波数成分ごとの強さの分布を求める手法であり、周波数分布の
偏りから音の明るさ・暗さを定量的に捉えられる。

ゴールの瞬間から**2秒後まで**の解析窓について、各クリップの平均スペクトルをクリップごとに
正規化したうえで群ごとに平均したものが以下のグラフ。
...
上記の図はスペクトルの分布そのものだが、これとは別に、同じ解析窓から算出したスペクトル重心・
ロールオフ85%・帯域幅について、Welchのt検定(等分散を仮定しない)でプレミア群とリーガ群を
比較した結果が以下:
...
として明るく、リーガは低域寄りでこもった/暗い音色**と言える。

### 4.3 フォルマント分析(母音特性)

歓声の母音的な音色(「ア」寄りか「イ」寄りかなど)を調べるため、フォルマント分析を行う。
フォルマント分析とは、声道の共鳴によって強調される周波数(フォルマント、特にF1・F2)を推定し、
声の母音としての特徴を捉える手法である。

ゴールの瞬間から**2秒後まで**の解析窓について、`praat-parselmouth`(PraatのPython版)のBurg法で
F1・F2を推定した。群衆音は単一話者を前提としたLPC解析には理論上不向きだが、「集団としての
スペクトル包絡の山」として近似的に解釈できる、という前提のもとで実施した(詳細は5章の限界を参照)。
...
包絡の形そのものを見るため、各クリップのLPC包絡(librosa自前のBurg法LPC。parselmouthとは独立な
実装であり、F1・F2の推定と完全に同一の計算ではないことに注意)を計算し、群ごとに正規化平均した
ものが以下:

![LPC包絡の形状](../results/loopback_comparison/formant_envelope.png)

1つ目の山(F1相当、800〜900Hz)はリーガの方が高く鋭く、2つ目の山(F2相当)はプレミアが
1600〜1700Hz付近、リーガが1400Hz付近とプレミアの方がやや高い。なお、プレミア側の1クリップで
...
ゴールの瞬間から**2秒後まで**の解析窓について、pYINアルゴリズムで基本周波数(ピッチ)を推定した
(有声フレーム率も記録)。クリップごとのF0中央値をWelchのt検定(等分散を仮定しない)でプレミア群と
リーガ群を比較した結果が以下:
...
ほぼ同じだった**。群衆のブロードバンドな歓声に対する推定自体が本質的に不安定な指標であり(5章
参照)、この程度のサンプル数では有意差が出にくいことは織り込み済みである。
...
ゴールの瞬間から**2秒後まで**の解析窓について、ゼロ交差率を求めた。クリップごとの値をWelchの
t検定(等分散を仮定しない)でプレミア群とリーガ群を比較した結果が以下:
...
歓声がどれだけ周期的・トーン的か(声が揃っているか)、それともノイズ的か(バラバラな唸り声か)を
調べるため、HNR(Harmonics-to-Noise Ratio)を算出する。HNRとは、波形の自己相関から求まる周期性の
強さを表す指標(dB)であり、高いほど周期的・トーン的、低い(あるいは負の)値ほどノイズ的である
ことを示す。

ゴールの瞬間から**2秒後まで**の解析窓について、parselmouth(Praatのcross-correlation法)で
フレームごとのHNRを求め、その中央値を算出した。各クリップのHNRの時間変化(太線=群平均、
薄線=個別クリップ):

![HNRの時間変化](../results/loopback_comparison/hnr_over_time.png)

プレミア側の1クリップで、自己相関法が短い区間で強い周期性を誤検出したと見られるスパイク
(30dB・25dB付近)が見えるが、中央値への影響はごくわずか。またリーガの群平均線は1.8秒付近で
途切れているが、これは3章で述べたレアル・マドリードの1クリップのデータ不足による。

クリップごとのHNR中央値についてWelchのt検定(等分散を仮定しない)でプレミア群とリーガ群を
比較した結果が以下:

| 指標 | プレミア (mean±SD) | リーガ (mean±SD) | p値 |
|---|---|---|---|
| HNR | -1.52 ± 0.69 dB | -0.39 ± 0.49 dB | 0.0023 |

有意差があり、**リーガの方が周期的・トーン的、プレミアの方がノイズ的**という結果になった。
「イェーイ(明るく弾ける)」対「ウォオ(低く唸る)」という最初の聴感上の印象からすると意外な
方向だが、4.2節のスペクトル形状(リーガは800〜900Hz付近に鋭く集中した山、プレミアは複数帯域に
広がったなだらかな形)とは整合的である -- 1つの帯域に集中した音の方が周期的に見えやすく、
複数帯域に広がった音の方がノイズ的に見えやすい、という説明ができる。
...
- **F0(ピッチ)推定の不安定性**: pYINによるピッチ推定は、群衆の歓声のような非周期的・
  ...
  計算された中央値が外れ値になりやすい
- **HNR推定の妥当性**: フォルマント推定と同様、HNRも本来1人の声の周期性を前提とした指標。
  群衆全体の音への適用は近似であり、真に「声が揃っているか」を測っているというより、音が
  特定の帯域に集中しているか広がっているかに近い性質を捉えている可能性がある
