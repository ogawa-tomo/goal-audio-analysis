...
歓声がゴールの瞬間にどれだけノイズ的な音色に変化するかを調べるため、ゼロ交差率を算出する。
ゼロ交差率とは、波形が正負をまたぐ頻度であり、値が高いほどノイズ性・高域成分が多いことを示す
指標である。

ゴールの瞬間の前2秒・後2秒それぞれのゼロ交差率を求め、その差分(後-前)も算出した。ゼロ交差率は
周波数ビンを持たない単一のスカラー値のため、スペクトルそのものを比較する術がなく、単純な前後
比較にとどめている。これらについてWelchのt検定(等分散を仮定しない)でプレミア群とリーガ群を
比較した結果が以下:

| 指標 | プレミア (mean±SD) | リーガ (mean±SD) | p値 |
|---|---|---|---|
| ゼロ交差率(ゴール後) | 0.130 ± 0.011 | 0.097 ± 0.005 | <0.0001 |
| ゼロ交差率(ゴール前) | 0.108 ± 0.014 | 0.102 ± 0.008 | 0.323 |
| ゼロ交差率の差分(後-前) | 0.022 ± 0.012 | -0.005 ± 0.008 | 0.0001 |

ゴール前の値には有意差がなく、両リーグの環境音自体の雑音性はほぼ同じだったことを示す。一方で
ゴール後の差分は明確に分かれており、**プレミアは9件全てでゴール後に値が上昇した(平均+0.022)の
に対し、リーガは平均でほぼ変化なし、むしろわずかに低下した(-0.005)**。プレミアの歓声はゴールの
瞬間に明確に雑音的な音色へ転じるのに対し、リーガの歓声は音色自体があまり変化しない(ゴール前後
で似た性質を保つ)ことを示しており、4.2節のスペクトル形状の図で見られた、プレミアが高域まで
幅広く音を伸ばす傾向を別の角度から裏付けている。
