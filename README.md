...
- **ループバック録音の場合は考え方が異なる点に注意**。録音は`loopback_record.py`実行後の
  ...
  「動画を一時停止する位置」で決まる。ここで一時停止位置を早めても、探索範囲(既定6秒)の
  終端はズレないため、必ずしも安全側には働かない(むしろゴールの着地点が探索範囲の後端に
  近づき、余裕が減る側に働く)。人間が再生ボタンを押すタイミングは早すぎるより遅れがちな
  ので、**一時停止位置は`--lead`の既定値通り(ゴールの3秒前)を基本とし、無理に早める必要は
  ない**。より安全を期したい場合は、一時停止位置を早める代わりに、`--duration`(録音時間)と
  `--peak-search-window`を両方増やす方が一貫性がある(上記の`--lead`と
  `--peak-search-window`の関係を参照)
