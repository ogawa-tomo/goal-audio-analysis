...
- **最低限、各ゴールの瞬間から見て前に3秒・後ろに6秒(既定値の場合)は音声が途切れて
  いないこと**。タイムスタンプの誤差(特に「気づいてから記録するまでの反応遅れ」で
  実際より遅めに記録しがちな点)も考慮すると、実際にはこれよりさらに余裕を持たせて
  おくと安全(例: 前に5秒、後ろに8〜10秒程度)。ループバック録音の場合は、ゴール発生が
  予想されるタイミングより数秒早めに録音を開始し、遅めに録音を止めるとよい
