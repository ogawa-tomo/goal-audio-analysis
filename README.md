...
- **`features.py`**: 1クリップの音声から音響特徴量(立ち上がり時間・スペクトル重心・
  ロールオフ・帯域幅・F0・フォルマントF1/F2)を抽出する。基準点はサイドカーファイル
  (`<clip>.mark.json`)に書かれた、人間が確認した2つの時刻: `human_marked_time_s`(音量が
  最も大きい瞬間。`scripts/mark_goal_moment.py`で指定)と`onset_marked_time_s`(歓声が
  盛り上がり始めた瞬間。立ち上がり時間・減衰時間は前者を、それ以外のスペクトル特徴量・
  増分分析は後者を基準にする)。**`onset_marked_time_s`を書き込む専用スクリプトはまだ
  リポジトリに整備できていない**(詳細な経緯・計算方法は`reports/premier_vs_laliga.md`の
  4.2節を参照)。どちらか一方でもマークされていないクリップは解析できない(エラーになる)
...
- **YouTube等の動画サイトが原音源の場合**: 多くのサービス(YouTube含む)は利用規約で
  ...
  - **録音した内容の中で、実際にどこが目的のイベント(ゴール)の瞬間かは、人間が指定する**。
    ...
    その時刻をサイドカーファイル(`<clip>.mark.json`)として保存する。`goal-audio analyze`は
    これを**そのまま**解析の基準点として使う。マークしていないクリップは解析できない(エラーに
    なる)。
...
解析の基準点は`mark_goal_moment.py`で人間が指定した時刻そのものであり、アルゴリズムによる
自動検出は行わない。したがって`extract-clip`に渡す**タイムスタンプの精度自体は重要ではない**
-- クリップのどこかにゴールの瞬間が収まっていれば、あとは`mark_goal_moment.py`で正確な時刻を
指定すればよい。

ただし、以下の点には注意する。

- **クリップには、ゴールの瞬間の前後に十分な余裕を持たせること**。基準点の前0.5秒・後2.5秒は
  スペクトル特徴量の解析窓として直接使われるため、最低限その範囲は必要。またゴール前の
  チャンスシーンなど紛らわしい展開が含まれる場合、それも含めて聴けた方が正しい瞬間を判断
  しやすいため、前後とも数秒の余裕を持たせておくのが望ましい(目安: 前に5秒、後ろに8秒程度)
...
# 3. ゴールの正確な瞬間を指定する(audio_time_reader.html等で確認してから)
python scripts/mark_goal_moment.py data/clips/goal1.wav 3.85
...
- 解析の基準点はマークされた時刻そのものであり、アルゴリズムによる自動検出・補正は一切
  行わない。マークの精度がそのまま解析結果の精度になるため、`mark_goal_moment.py`の
  docstringにある通り、慎重に確認してから確定すること
