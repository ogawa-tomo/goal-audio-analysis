...
- **Creative Commons等でライセンスが明示された音源(Freesoundなど)の場合**: ライセンス条件
  (帰属表示の要否、商用利用可否など)を確認した上でダウンロードして問題ない。使用した音源の
  出典・ライセンスは`data/raw/`内に記録しておくとよい(音源ファイル自体はGit管理外)。
  このように手元に既に長い音声ファイルがある場合は、
  [`scripts/extract_clip.py`](scripts/extract_clip.py)で短いクリップを切り出せる
  (`python scripts/extract_clip.py <元ファイル> <出力先> <center秒数>`。`ffmpeg`が必要)
...
したがって`scripts/extract_clip.py`に渡す**タイムスタンプの精度自体は重要ではない** --
クリップのどこかにゴールの瞬間が収まっていれば、あとで正確な時刻を指定すればよい。
...
- 以前のバージョンではクリップ切り出し機能(現`scripts/extract_clip.py`)に`yt-dlp`を使った
  YouTube直接ダウンロード機能を含んでいたが、YouTubeの利用規約(公式のダウンロード機能以外での
  ダウンロード禁止)に抵触する行為を助長するコードであったため**削除した**。音声データの
  用意方法は上記「音声データの用意について」を参照
