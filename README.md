...
1. `extract-clip`が、タイムスタンプの**`--lead`秒前から`--duration`秒間**(既定値:
   `--lead 1.0` `--duration 7.0`)を切り出す。つまり既定値では
   **[タイムスタンプ - 1秒, タイムスタンプ + 6秒]** の範囲が切り出される
2. 続く`analyze`が、切り出されたクリップの**先頭4秒以内**で実際に音量(RMS)が最大になる
   瞬間を自動検出し、それを基準点として立ち上がり時間・スペクトル特徴量・F0・フォルマントを
   算出する(詳しくは後述「既知の注意点」を参照)

**したがって、用意する音声データには最低限、ゴールの瞬間から見て「前に1秒・後ろに6秒」分が
含まれている必要がある**(既定値の場合。それより前後が欠けていると、そもそも`extract-clip`
がその範囲を切り出せない)。

その上で、2.のピーク検出により**タイムスタンプ自体は秒単位で厳密でなくても問題ない**。目安
として、与えたタイムスタンプの**-1秒〜+3秒の範囲内**に実際の歓声のピークが収まっていれば、
...
- **最低限、各ゴールの瞬間から見て前に1秒・後ろに6秒(既定値の場合)は音声が途切れて
  いないこと**。タイムスタンプの誤差や、上記のピーク誤検出のリスクも考慮すると、
  実際にはこれよりさらに数秒の余裕を持たせておくと安全(例: 前に3〜5秒、後ろに8〜10秒程度)。
